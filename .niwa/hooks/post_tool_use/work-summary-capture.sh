...
#
# Pure pass-through: all logic (ledger, emit gate, render, sanitizer, hook-JSON
# shaping) lives in the on-PATH `shirabe` binary as `shirabe work-summary
# capture`. The binary reads the raw hook JSON from stdin, extracts what it needs
# itself, always exits 0 (fail-safe), and prints the COMPLETE hook JSON to stdout
# (or nothing when suppressed). This hook only routes stdin/stdout.
#
...
exec shirabe work-summary capture
